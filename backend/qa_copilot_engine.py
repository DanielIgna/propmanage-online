...
async def analyze_finding(finding_text: str, *, role: str, area: str, prior_findings: list[dict]) -> dict:
    ...
    chat = _llm_chat(_ANALYZE_SYS, f"qa-copilot-analyze-{uuid.uuid4().hex[:8]}")
    if chat is None:
    ...
    try:
        ...
        return {
            ...
            "suspected_files": [str(x)[:200] for x in (parsed.get("suspected_files") or [])][:6],
