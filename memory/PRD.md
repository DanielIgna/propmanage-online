...
- **Phase 47C (Repair Audit Log) — Feb 2026 — verified live**:
  - **Backend**: `GET /api/admin/ai/repair-suggestions/audit?days=N` aggregates per-pattern effectiveness via Mongo pipeline ($group on `finding_pattern`): total / proposed / approved / applied / rejected / approve_rate_pct / reject_rate_pct / apply_rate_pct / **effectiveness_pct** (applied / decided), avg_minutes, avg_regenerations, high_risk count. Also returns `totals` (global) + `best_pattern` + `worst_pattern` (only patterns with ≥3 decisions qualify). `GET /api/admin/ai/repair-suggestions/by-pattern/{pattern}?days=` for drill-down.
  - **Frontend**: New `pages/admin/RepairAuditLog.jsx` mounted between Findings and Chat in AdminAIConsole. Shows global 5-stat header, Best/Worst pattern cards (green/red), per-pattern table with: pattern label + raw id, total/applied/approved/rejected columns (color-coded), apply-rate progress bar, eficacitate with TrendingUp/Down icons (≥70% good, <30% bad), avg_minutes + avg_regenerations, click-row drill-down modal showing all suggestions for that pattern with status/risk badges and decision notes.
  - 7d / 30d / 90d window selector. All testids exposed (`audit-window-N`, `repair-audit-rows`, `audit-row-{pattern}`, `audit-drill-modal`, `audit-drill-item-{id}`).
