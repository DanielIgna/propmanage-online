...
import { FileText, Send, Download, Eye, X, Plus, Users, History, Loader2 } from "lucide-react";
  ...
  return (
    <div className="space-y-5" data-testid="admin-docs-panel">
      <AdminCard
        ...
        action={<FileText className="w-3.5 h-3.5 text-slate-400" />}
      >
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {docs.map(d => (
              <div key={d.slug} className="rounded-2xl border border-slate-200 dark:border-slate-700 p-4 bg-white dark:bg-slate-900" data-testid={`docs-card-${d.slug}`}>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setPreviewSlug(d.slug)} className="flex-1 text-xs flex items-center justify-center gap-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg py-2" data-testid={`docs-preview-${d.slug}`}>
                    <Eye className="w-3 h-3" /> Preview
                  </button>
                  <a href={`${API}/admin/docs/${d.slug}/pdf`} target="_blank" rel="noopener noreferrer" className="text-xs flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg py-2 px-3" data-testid={`docs-pdf-${d.slug}`}>
                    <Download className="w-3 h-3" /> PDF
                  </a>
                  <button onClick={() => setSendSlug(d.slug)} className="flex-1 text-xs flex items-center justify-center gap-1.5 bg-[#d4ff3a] text-black font-semibold hover:bg-[#bfe632] rounded-lg py-2" data-testid={`docs-send-${d.slug}`}>
