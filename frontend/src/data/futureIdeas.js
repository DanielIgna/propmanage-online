    ...
    phases: [
      {
        ...
        description: "✅ DONE (Feb 2026). Definire model + registry hardcoded (13 acțiuni critice) + storage pentru founder contact în app_settings. Feature flag `enable_founder_gate` = false (foundation only).",
        deliverables: [
          "Model `FounderContact` în app_settings (email, backup_email, phone) — DONE",
          "Module `founder_gate/registry.py` cu 13 acțiuni hardcoded (financial/data/security/governance) — DONE",
          "Helper `is_critical_action(slug)` reutilizabil — DONE",
          "Routes `/api/admin/founder-gate/status` + `/critical-actions` — DONE",
          "Read-only UI `/admin/founder-gate` — DONE",
        ],
      },
      {
        code: "FG-1", title: "Twilio Integration + SMS Service (⏸️ DEFERRED)", days: 2,
        description: "🟠 DEFERRED — User-decisia (Feb 2026): NU se integrează Twilio momentan. Pas amânat până după testarea în beta și onboarding-ul primilor clienți reali. NU se creează cont Twilio, NU se adaugă dependențe Twilio. Re-evaluare după validarea beta.",
        deliverables: [
          "[BLOCKED] Cont Twilio (free trial sau plătit) + RO sender number",
          "[BLOCKED] Helper `send_verification_sms(phone, code)` cu retry 3x",
          "[BLOCKED] Rate limit 5 SMS/oră/founder",
          "[BLOCKED] Alert email când rate exceeded sau Twilio fails",
          "[BLOCKED] Fallback: email TOTP (Authenticator app) dacă SMS eșuează",
        ],
      },
      {
    ...
    openQuestions: [
      {
        ...
        founderDecision: "b) Adaugă: modificare credențiale Stripe, API keys, schimbare email founder (→ 12 acțiuni)",
      ...
      },
      {
        ...
        options: [
          ...
          "d) ⏸️ DEFERRED — Twilio NU se integrează acum. Re-evaluare după testare beta + clienți reali.",
        ],
        founderDecision: "d) ⏸️ DEFERRED — Twilio NU se integrează acum. Re-evaluare după testare beta + clienți reali. Confirmat: nu se creează cont Twilio, nu se adaugă dependențe, nu se modifică DNS.",
      ...
      },
      {
        ...
        founderDecision: "a) 24h (recomandat) — auto-reject după zi întreagă, founder are timp",
        decidedAt: "2026-06-05",
      },
      {
        ...
        founderDecision: "a) ACUM — START Phase FG-0 (foundation) imediat după aprobare (programat pentru când founder are credite Emergent)",
        decidedAt: "2026-06-05",
      },
    ],
  },
